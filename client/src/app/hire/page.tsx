...
function HireContent() {
    ...
    const handleNextStep = () => {
        ...
        if (step === 2) {
            ...
            // Instead of setStep(3), we trigger registration
            handleRegister();
        }
    };
                            ...
                            {step === 1 && (
                                ...
                                >
                                    ...
                                    <div className="grid gap-4">
                                        {[
                                            { id: 'basic', name: 'Apprentice', price: '$29', features: ['3 Talents/mo', 'Email Support'] },
                                            { id: 'professional', name: 'Professional', price: '$59', features: ['10+ Talents', '1 Team', 'Priority Support', 'Top 10% Talent'], popular: true },
                                            { id: 'enterprise', name: 'Enterprise', price: '$199', features: ['Unlimited Talents/Team', '1 Agency', 'Custom Contracts', 'Top 3% Talent'] }
                                        ].map((plan) => (
                                            <div
                                                key={plan.id}
                                                onClick={() => setSelectedPlan(plan.id)}
                                                className={`relative p-6 text-black rounded-xl border-2 cursor-pointer transition-all ${selectedPlan === plan.id ? 'border-[#204ecf] bg-blue-50/50' : 'border-gray-100 hover:border-gray-200'
                                                    }`}
                                            >
                                                {plan.popular && (
                                ...
                                >
                                    ...
                                    <div className="space-y-4 text-black">
                                        <div>
                                            ...
                                            <Input className='text-black bg-gray-100' id="companyName" name="companyName" value={formData.companyName} onChange={handleInputChange} placeholder="Acme Inc." />
                                        </div>
                                        <div>
                                            <Label htmlFor="full_name">Full Name</Label>
                                            <Input className='text-black bg-gray-100' id="full_name" name="full_name" value={formData.full_name} onChange={handleInputChange} placeholder="John Doe" />
                                        </div>
                                        <div>
                                            <Label htmlFor="email">Work Email</Label>
                                            <Input className='text-black bg-gray-100' id="email" name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="john@acme.com" />
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <Label htmlFor="password">Password</Label>
                                                <Input className='text-black bg-gray-100' id="password" name="password" type="password" value={formData.password} onChange={handleInputChange} />
                                            </div>
                                            <div>
                                                <Label htmlFor="confirmPassword">Confirm Password</Label>
                                                <Input className='text-black bg-gray-100' id="confirmPassword" name="confirmPassword" type="password" value={formData.confirmPassword} onChange={handleInputChange} />
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex gap-4">
                                        <Button variant="default" onClick={() => setStep(1)} className="w-1/3 text-white bg-[#204ecf]">Back</Button>
                                        <Button onClick={handleRegister} className="w-2/3 bg-[#204ecf] hover:bg-[#1a3da8] text-white font-bold py-6" disabled={isLoading}>
                                            {isLoading ? (
